      ...
      {profiles.map((profile) => (
        <div key={profile.id} className="bg-[#1A0033]/50 border border-white/10 rounded-xl p-6 backdrop-blur-sm hover:border-[#FBAE17]/50 transition-colors">
          ...
          <div className="flex gap-3 pt-4 border-t border-white/10">
            {profile.github_url && (
              <a href={normalizeHref(profile.github_url)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" title="GitHub">
            ...
            {profile.linkedin_url && (
              <a href={normalizeHref(profile.linkedin_url)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#0a66c2] transition-colors" title="LinkedIn">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" /></svg>
              </a>
            )}
            {profile.portfolio_url && (
               <a href={normalizeHref(profile.portfolio_url)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#FBAE17] transition-colors" title="Portfolio">
