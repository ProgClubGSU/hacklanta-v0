import { wrapInMinimalLayout } from './layout-minimal'
...
export const templates = {
  acceptance(data: RecipientData): TemplateResult {
    ...
    return {
      ...
      html: wrapInMinimalLayout(`
        ${p(`${g}`)}
        ${p(`You made the cut. Your spot at Hacklanta is confirmed.`)}
        ${p(`Head to your ${link('dashboard', DASHBOARD_URL)} to set up your profile and find a team. Spots are limited and we have a waitlist — if you don't confirm, someone else gets your spot.`)}
        ${p(`12 hours. One night. $5,000 prize pool.`)}
        ${sig()}
      `),
      text: `${g}
...
We squeezed you in — you're an overflow admit. Full access to the event, same 12 hours, same experience.
